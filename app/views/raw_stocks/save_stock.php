...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        // Get next stock_id
        $result = $conn->query("SELECT MAX(stock_id) AS max_id FROM stock_entries_raw");
        $row = $result->fetch_assoc();
        $next_stock_id = $row['max_id'] ? $row['max_id'] + 1 : 10001;

        // Insert stock entry
        $sql = "INSERT INTO stock_entries_raw (supplier_id, stock_id, itemcode, product_name, purchase_qty, unit, available_stock, cost_price, wholesale_price, max_retail_price, super_customer_price, our_price, low_stock, expire_date, discount_percent, free, gift, voucher, barcode, total_cost_amount, branch) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $stmt->bind_param("ssssdsddddddssdiiisds", 
            ...
            $free, $gift, $voucher, $itemcode, $total_amount, $current_branch_stock
        );
        ...
        // Update available_stock in raw_items
        $updateStockSql = "UPDATE raw_items SET available_stock = available_stock + ? WHERE item_code = ?";
        $updateStockStmt = $conn->prepare($updateStockSql);
        $updateStockStmt->bind_param("ds", $purchase_qty, $itemcode);

        if (!$updateStockStmt->execute()) {
            throw new Exception("Error updating available stock in raw_items: " . $updateStockStmt->error);
        }

        // Commit transaction
        $conn->commit();
        echo "Stock saved successfully, and available stock updated.";
