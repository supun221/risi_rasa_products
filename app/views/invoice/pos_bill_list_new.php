...
$billId = $billData['bill_id'] ?? $billData['billId'] ?? 'N/A';
$currentDate = $billData['bill_date'] ?? $billData['currentDate'] ?? date("Y-m-d H:i:s");
$productList = $billData['productList'] ?? [];
$grossAmount = $billData['gross_amount'] ?? $billData['grossAmount'] ?? 0;
$cashTendered = $billData['cashTendered'] ?? 0;
$netAmount = $billData['net_amount'] ?? $billData['netAmount'] ?? 0;
$advancePaymentAmount = $billData['advancePaymentAmount'] ?? 0;
$discountAmount = $billData['discount_amount'] ?? $billData['discountAmount'] ?? 0;
$paymentType = strtoupper(str_replace("_", " ", $billData['payment_type'] ?? $billData['paymentType'] ?? 'CASH'));
...
$customerID = $billData['customer_id'] ?? $billData['customerID'] ?? 0;
...
<body onload="window.print()">
    ...
    <table>
        ...
        <tbody>
            ...
            <?php foreach ($productList as $index => $product): ?>
                <?php
                if(isset($product['freeIssueAmount']) && $product['freeIssueAmount'] > 0){
                    $freeIssuesFlag = true;
                }
                $productName = $language === 'sinhala' ? getSinhalaProductName($product['product_name'] ?? $product['productName'] ?? '') : ($product['product_name'] ?? $product['productName'] ?? '');
                ?>
                <tr>
                ...
                <tr>
                    ...
                    <td style="text-align: left; font-size: 14px; margin-top: -14px; vertical-align: top;">
                        <strong><?= formatAmount($product['price'] ?? $product['unitPrice'] ?? 0); ?></strong>
                    </td>
                    <td style="text-align: center; margin-top: -14px; vertical-align: top;">
                        <?= htmlspecialchars($product['purchase_qty'] ?? $product['quantity'] ?? 0); ?>X
                    </td>
                    <td style="text-align: center; font-weight: bold; vertical-align: top;">
                        <?= formatAmount($product['price'] ?? $product['ourPrice'] ?? 0); ?>
                    </td>
                    <td style="text-align: center; vertical-align: top;">
                        <?= formatAmount($product['subtotal'] ?? 0); ?>
                    </td>
                </tr>
                <?php $totalItems += ($product['purchase_qty'] ?? $product['quantity'] ?? 0); ?>
            <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <?php if($freeIssuesFlag): ?>
        ...
        <table class="fr-issue-table" style="border-bottom: 2px solid black;">
            <?php foreach ($productList as $product): ?>
                <?php if(isset($product['freeIssueAmount']) && $product['freeIssueAmount'] > 0): 
                    $productName = $language === 'sinhala' ? getSinhalaProductName($product['product_name'] ?? $product['productName'] ?? '') : ($product['product_name'] ?? $product['productName'] ?? '');
                ?>
    ...
    <table class="summary">
        ...
        <tr>
            ...
            <td><?= formatAmount(($grossAmount-$netAmount) - $advancePaymentAmount); ?></td>
        </tr>
        <?php 
            if($advancePaymentAmount > 0){
                echo '
                    <tr>
                        <td class="'. ($language === 'sinhala' ? 'sinhala-text' : 'english-text') .'">
                            '. ($language === 'sinhala' ? getSinhalaText('Advance Payment') : 'Advance Payment') .'
                        </td>
                        <td>' . formatAmount($advancePaymentAmount) . '</td>
                    </tr>
                ';
            }
        ?>
