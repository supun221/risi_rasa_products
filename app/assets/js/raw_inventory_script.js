...
document.addEventListener("DOMContentLoaded", () => {
  ...
  saveButton.addEventListener("click", () => {
    ...
    const payload = { category, productName, sinhalaProductName, itemCode, mode, productPrice, productWPrice, productMRPrice };
...
function fetchAllStocksRaw() {
    fetch(`get_all_stocks_raw.php?page=${currentPage}&limit=${itemsPerPage}`)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
                totalRecords = data.total;
                displayAllStocksRaw(data.data);
                createPagination();
            } else {
                console.error('Error:', data.message);
                showErrorRaw('Failed to load stocks');
            }
        })
        .catch(error => {
            console.error('Error:', error);
...
function displayAllStocksRaw(stocks) {
    const tableBody = document.getElementById('allStockTableBodyRaw');
    ...
    stocks.forEach(stock => {
        ...
        row.innerHTML = `
            ...
            <td>${stock.itemcode}</td>
            <td>${stock.product_name}</td>
            <td>${formatPrice(stock.cost_price)}</td>
            <td>${stock.purchase_qty}</td>
...
function createPagination() {
    ...
    paginationContainer.innerHTML = `
        <div class="pagination">
            ...
            <span class="page-info">Page ${currentPage} of ${totalPages} (Total: ${totalRecords})</span>
...
document.addEventListener('DOMContentLoaded', fetchAllStocksRaw);