...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    notifier.alert("Error fetching raw materials:", error);
  }
});

const populateInitialRawMaterialDropdown = () => {
  const dropdown = document.getElementById("raw-ingredient-selector-0");
  ...
  dropdown.addEventListener("change", function () {
    ...
    if (selectedEntry) {
      document.getElementById("raw-material-name-0").value =
        selectedEntry.product_name;
      document.getElementById("raw-material-rem-stock-0").value =
...
const addRawIngredientRow = () => {
  ingredientRowCount++;
  const container = document.querySelector(".si-content-upper");
  const newRow = document.createElement("div");
  newRow.classList.add("raw-mat-selection");
  newRow.id = `raw-ingredient-row-${ingredientRowCount}`;

  newRow.innerHTML = `
    <div class="input-field-cont">
      <label class="si-prod-label">Raw Material</label>
      <select class="si-prod-input" id="raw-ingredient-selector-${ingredientRowCount}">
        <option value="null" selected>Select a raw material</option>
      </select>
      <input type="text" id="raw-material-name-${ingredientRowCount}" hidden>
      <input type="text" id="raw-material-rem-stock-${ingredientRowCount}" hidden>
    </div>
    <div class="input-field-cont">
      <label class="si-prod-label">Input Amount (in Kilograms)</label>
      <input class="si-prod-input" type="number" id="raw-material-inp-amount-${ingredientRowCount}" oninput="amountValidator(${ingredientRowCount})" value="0">
      <p class="rem-amount-info" id="rem-amount-info-${ingredientRowCount}"></p>
    </div>
    <button class="remove-ingredient-btn" onclick="removeRawIngredientRow(${ingredientRowCount})">❌</button>
  `;

  container.appendChild(newRow);

  const dropdown = document.getElementById(
    `raw-ingredient-selector-${ingredientRowCount}`
  );
  raw_materials.forEach((entry) => {
    let option = document.createElement("option");
    option.value = entry.id;
  ...
  dropdown.addEventListener("change", function () {
    const rowIndex = this.id.split("-").pop();
    const selectedId = this.value;
    const selectedEntry = raw_materials.find((entry) => entry.id == selectedId);

    if (selectedEntry) {
      document.getElementById(`raw-material-name-${rowIndex}`).value =
        selectedEntry.product_name;
      document.getElementById(`raw-material-rem-stock-${rowIndex}`).value =
        selectedEntry.available_stock;
    }
    materialValidator(rowIndex);
  });
};

const removeRawIngredientRow = (rowIndex) => {
  const rowToRemove = document.getElementById(`raw-ingredient-row-${rowIndex}`);
  if (rowToRemove) {
    rowToRemove.remove();
  }
  filterToggler();
...
const materialValidator = (rowIndex) => {
  const remainderPlaceHolder = document.getElementById(
    `rem-amount-info-${rowIndex}`
  );
  const remainderValue = document.getElementById(
    `raw-material-rem-stock-${rowIndex}`
  ).value;
  const materialSelector = document.getElementById(
    `raw-ingredient-selector-${rowIndex}`
  ).value;
...
const amountValidator = (rowIndex) => {
  const remainderValue = document.getElementById(
    `raw-material-rem-stock-${rowIndex}`
  ).value;
  const amountPlaceHolder = parseFloat(
    document.getElementById(`raw-material-inp-amount-${rowIndex}`).value
  );
...
const filterToggler = () => {
  ...
  // Check all raw material inputs
  for (let i = 0; i <= ingredientRowCount; i++) {
    const rawMaterialSelector = document.getElementById(
      `raw-ingredient-selector-${i}`
    );
    const amountInput = document.getElementById(`raw-material-inp-amount-${i}`);

    if (rawMaterialSelector && amountInput) {
      if (
        parseFloat(amountInput.value) <= 0 ||
        rawMaterialSelector.value === "null"
      ) {
...
const executeProduction = () => {
  ...
  createProductionOrder(
    productionOrderId,
    date,
    rawMaterialData.reduce((sum, mat) => sum + mat.inputAmount, 0),
    totalWeight,
...
async function createProductionOrderItem(
  production_order_id,
  item_name,
  barcode,
  quantity,
...
async function updateRawMaterialStock(id, new_stock) {
  try {
